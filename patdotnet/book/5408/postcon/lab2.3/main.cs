...
namespace ConsoleCalculator
{
        ...
        public int process( int runningTotal, int value) {
            ...
            foreach( Step item in _list) {
                if( item._op == CalculatorFullBridgeWithState.Ops.Recall) {
                    ...
                    // It is partially ok, because it is a single change.  However, if this
	...
	class Class1
	{
		...
		{
            // Notice how the builder creates a class of type IOperation.
            // Using this approach the end user can integrate the IOperation
            // implementation without to re-invent a new data type.
            IOperation op = Builder.createOperationType1( 2, 3, 4, 7);
            Console.WriteLine( "Result is " + op.process( 0, 0));
		}
	}
}
