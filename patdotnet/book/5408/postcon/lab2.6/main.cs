...
namespace ConsoleCalculator
{
    ...
    class FormulaToOperationsAdaptor : IOperation {
        IFormula _formula;

        public FormulaToOperationsAdaptor() {
            _formula = new ImplFormula( new SafeSquareRootProxy( new ImplFormula( new EndPoint(), 1, 1)), 2, 2);
        }
        public int process(int runningTotal, int value) {
            return _formula.process( runningTotal, value);
        }
    }
